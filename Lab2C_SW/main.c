...
int main(void) {
	...
	for (;;) {
		while (i == (P1IN & 0x3)) // Is switch status different then last print?
			;
		i = P1IN & 0x3; // Get only the switch portion of register

		// Convert Switch status to a char and update string
		// Treating button status as an integer to offset '0' char
		str[6] = '0' + (i & 1); // SW1 status char position
		str[15] = '0' + (i >> 1 & 1); // SW2 status char position

		OUTA_STR_UART(str); // Print string
	}
}
